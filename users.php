...
else if(isset($_REQUEST['id']) || isset($_REQUEST['id1']))
{
	...
	$query="update ev_user_role SET status='$status' WHERE user_role_id='$id'";         
	mysqli_query($conn,$query);

}
else if(isset($_REQUEST['2fid']) || isset($_REQUEST['2fid1']))
{
	if(isset($_REQUEST['2fid']))
	{
	$id2f=$_REQUEST['2fid'];
	$status='inactive';
	}
	else if(isset($_REQUEST['2fid1']))
	{
	$id2f=$_REQUEST['2fid1'];	
	$status='active';
	}
	...
	$query="update qr_users SET 2fa_switch='$status' WHERE id='$id2f'";         
	mysqli_query($conn,$query);
        ...
        <div class="col-12">
          <div class="card">
            <div class="card-body">
              <div class="row">
                <div class="col-6">
                  <h4 class="card-title">User Privilege List</h4>
                  <small class="form-text text-muted mb-3">*This table displays all <b>User Privilege</b> </small><br>
                </div>
                <div class="col-6"> 
                  ...
                  <a type="button" class="btn btn-info btn-sm" href="privileges-form.php" style="float: right;"><i class="fas fa-plus"></i> Add Privilege</a> </div>
              </div>
            ...
            selectedRow();
                ...
                <div onscroll='scroller("scrollme", "scroller")' id="scrollme">
                  <table id="multi_col_order" width="100%">
                    ...
                    <tbody>
                      <?php
                      $sql2 = "SELECT * FROM qr_users ORDER BY id DESC";
                      ...
                      <tr>
                        ...
                        <td><?php echo $row2['username'];?></td>
						  ...
						  <td><?php 
							$user_role_sql = mysqli_query( $conn, "select * from ev_roles WHERE role_id='$roleid'");
  							$list_role_sql = mysqli_fetch_array( $user_role_sql );
							if(mysqli_num_rows($user_role_sql)>0){
							echo $list_role_sql['role_name'];
							}
							  ?></td>
						  <td><?php echo $row2['useremail'];?></td>
						  <td><?php echo $row2['created_date'];?></td>
						  <td>
						  <?php
							$usname=$row2['username'];
							$username_sql = mysqli_query( $conn, "select * from ev_user_role WHERE username='$usname'");
  							$list_username = mysqli_fetch_array( $username_sql );
							  ?>
							  <?php if($list_username['status']=='aproved'){?>
							  <a href="users.php?id=<?php echo $list_username['user_role_id'];?>" onClick="return confirm('Do you really want to Inactive it?')">Approved</a>
							  <?php } else {?>
							  <a href="users.php?id1=<?php echo $list_username['user_role_id'];?>" onClick="return confirm('Do you really want to Approve it?')">Pending</a>
							  <?php } ?>
						  </td>
						  <td>
							  <?php if($row2['2fa_switch']=='active'){?>
							  <a href="users.php?2fid=<?php echo $row2['id'];?>" onClick="return confirm('Do you really want to Inactive it?')">Active</a>
							  <?php } else {?>
							  <a href="users.php?2fid1=<?php echo $row2['id'];?>" onClick="return confirm('Do you really want to Active it?')">Inactive</a>
							  <?php } ?>
						  </td>
                        <td id="status"> <a href="users.php?del=<?php echo $row2['id']; ?>" onClick="return confirm('Do you really want to remove it?')"><span class="fa fa-trash-alt"></span></a></td>
                      </tr>
                      <?php
                      }
                      } else {
