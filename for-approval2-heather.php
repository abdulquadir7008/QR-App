...
$sql = "SELECT 
            ...
            SUM(CASE WHEN state = 'Declined' THEN 0 ELSE 0 END) AS decline_total,
    ...
    if ($result2->num_rows > 0) {
        while ($row2 = $result2->fetch_array()) {
            ...
            <tr ondblclick="//location.href='edit-delegate.php?id=<?php //echo $row2['id']; ?>'">
                ...
                <?php
                $sql7 = "SELECT * FROM qr_users WHERE id = ?";
                $stmt = $conn->prepare($sql7);
                $stmt->bind_param("i", $approvedBy); //paramater binding added by heather
                $stmt->execute();
                ...
                if ($result7->num_rows > 0) {
                    while ($row7 = $result7->fetch_array()) {
                        $id = $row7['id'];
                        ?>
                        <td id="approve"><?php echo $row7['fullname']; ?></td>
                        <td id=""></td>
                        <?php
                    }
                } else {
                    ...
                    <td id="status">
                        ...
                        if ($login_access['fullname'] === $row2['created_by']) {
                            echo '<button class="btn btn-light btn-sm" disabled>Disabled</button>';
                        } elseif (strpos($row2['state'], 'Approved') !== false) {
                            echo '<button class="btn btn-light btn-sm" disabled>Approved</button>';
                        } else {
